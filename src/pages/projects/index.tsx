...
const formatProjectSlide = (project: ProjectEntity, files: ProjectFileEntity[]): ProjectSlide => {
  return {
    project,
...
const getProjectSlides = async (): Promise<ProjectSlide[]> => {
  ...
  const projectFiles: ProjectSlide[] = await Promise.all(projects.map(async (project) => {
    let files: ProjectFileEntity[] = await projectFilesMiddleware.retrieves(project.id);
