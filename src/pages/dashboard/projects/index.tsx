...
export default function Projects() {
  ...
  const handleDeleteProjectFile = async (projectFile: ProjectFileEntity) => {
    const response = await fetch(
      `/api/project/${projectFile.project.id}/file/${projectFile.id}`,
      {
        method: "DELETE",
      }
    );
    ...
    const newProjectFiles = projectFiles.filter(
      ({ id }) => id !== projectFile.id
    );
  ...
  const handleDeleteProjectTech = async (projectTech: ProjectTechEntity) => {
    const response = await fetch(
      `/api/project/${projectTech.project.id}/tech/${projectTech.id}`,
      {
        method: "DELETE",
      }
    );
    ...
    const newProjectTechs = projectTechs.filter(
      ({ id }) => id !== projectTech.id
    );
                      ...
                      <input
                        onChange={async (e) => {
                          ...
                          try {
                            ...
                            const result: ProjectFileEntity =
                              await response.json();
            ...
            {tab === TabsEnum.Techs && (
              <>
                ...
                <section>
                  ...
                  >
                    {techs
                      .filter((tech) => !techAlreadyExists(tech))
                      .map((tech) => (
                        <li key={tech.id + "tech"}>
                          <Chip
                            label={tech.name}
